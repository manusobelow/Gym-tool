...
//              you're adding a rung to an existing ladder (also update home-workouts.js's LADDERS entry).
//   n        — display name, Title Case.
//   p        — one of: "Squat","Hinge","Lunge","Push Horizontal","Pull Horizontal","Push Vertical",
//              "Pull Vertical","Core / Carry","Isolation","Warm-up / Mobility","Conditioning"
//              (must exactly match one of these strings — this is also the section banner below).
//   scheme   — "main" (major lift), "strength", "hypertrophy", "mobility", or "circuit" (Home
//              Workout only). If this is a Home Workout exercise, also wire its id into
//              home-workouts.js (LADDERS / MAJOR_LIFT_DAYS / CIRCUIT_DAYS as appropriate).
//   eq       — array from: barbell, dumbbell, kettlebell, cable, machine, bodyweight, band,
//              trapbar, landmine, rings. Bodyweight-only (["bodyweight"]) unlocks the Mobility
//              scheme option in the app's scheme picker.
//   m        — short muscle text shown on the card, e.g. "Quads / Glute Max".
//   region   — "upper" or "lower" only.
//   gif      — demo URL, or "" if none yet.
//   notes    — coaching cue text, or "" if none.
//   muscles  — [{id, w}] array. id must match a key in MUSCLE_LABELS (app-muscle-map.js) to show
//              up on the Muscle Map. w: 3 = primary mover, 2 = secondary, 1 = tertiary/stabilizer.
//
// Where to put it: append the new entry inside the matching "// ---------- PATTERN ----------"
// section below (order within a section doesn't matter functionally — grouped by pattern purely so
// a section can be found and read with one grep + one narrow view instead of scanning the whole file).
const EXERCISES = [
  // ---------- SQUAT (11) ----------
  {
    "id": "BB_BACK_SQUAT",
